...
export default function ExplainPage() {
  ...
  useEffect(() => {
    const fetchExplain = async () => {
      setLoading(true);
      try {
        const res = await apiClient.get('/api/explain/latest');
  ...
  // Guard - check for predictions instead of trained model
  if (loading) {
    return (
      <div>
  ...
  if (error || shapData.length === 0) {
    return (
      <div>
        <div className="page-header"><h1>Explainability</h1></div>
        <div className="card">
          <div className="alert alert-warning">
            No predictions generated yet. Please <a href="/predict" style={{ color: 'inherit', fontWeight: 700 }}>generate a prediction</a> to get insights.
          </div>
        </div>
      </div>
    );
    ...
    <div>
      ...
      {lastPrediction && (
        <div className="alert alert-info" style={{ marginBottom: 20 }}>
          Explaining prediction for last property estimate - Model: {bestModel?.name || 'Unknown'}
        </div>
      )}
